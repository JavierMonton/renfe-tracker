...
Because all this information that is obtained doing queries to the original website are very interesting, the application saves it in a database for future comparisons. In particular, each price observed (during search or during the scheduler) is stored as an individual record linked to the trip; ranges are computed when showing them (min and max of stored prices). If the same price is seen again, only the "last seen at" timestamp is updated (see "Estimated price range for tracked trips"). 
...
Then, the application will save this trip in the database, it will appear in the initial page, and it will run a query to the original website using the defined schedule, to find the real price. If the real price drops or goes up, it will detect it. When it detects this, it will save it to the database, related to this trip tracked. On every run of the scheduled job, the application updates a "last checked at" timestamp for that trip; a new price-change event is only stored when the price actually changes, but "last checked at" is always updated so the user knows when the trip was last queried. 

### Seeing tracked trains
If the user, in the initial page, clicks on a tracked trip, the application will show the events that it has detected. Events are price changes. So if the application know that prices changed a few times, it will show a row for each price change, and the date when it was detected. The detail view (and the list of tracked trips) also show "last checked at": the date and time when the scheduled process last ran for that trip. If the job has not run yet for that trip, it shows "not checked yet".

### Estimated price range for tracked trips
When searching for trips, the application shows an estimated price range (e.g. "prices usually 30€–70€") by checking the same train (same weekday, same departure time) on other dates. When a trip is tracked, we must keep showing this estimated range in the detailed view (and optionally in the list). To do that:

- **Saving prices from search:** The individual prices found during a search (requested date + same-weekday reference dates) must be saved in the database. They are stored as **individual records**, one per price observed, not as a single min–max range. Ranges are computed when displaying (min and max of stored prices).
- **Showing the range on tracked trips:** The detailed view of a tracked trip (and the list if desired) must show the estimated price range, computed from all stored price samples for that trip.
- **Scheduler improving the range:** The scheduler does not only check the current price for the trip’s date. It also runs the same logic used in search: same train (same weekday, same departure time) on **other weeks**, and records each price found. So over time the application collects more price points for that trip. More data does not necessarily mean a narrower range; it means a better-informed range (more data points).
- **Storage rules:** All stored prices are kept as **individual records**. If the same price is seen again (e.g. same trip, same price on a later scheduler run), we do **not** insert a duplicate; we update the **timestamp of when that price was last seen**. So the database has one row per (trip, price) with a "last seen at" timestamp. This way the estimated range for a commonly checked trip improves over time and remains understandable.
