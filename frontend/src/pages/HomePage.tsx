...
function formatCheckInterval(t: TFunction, minutes?: number | null) {
  if (!minutes || minutes <= 0) return '—'
  if (minutes < 60) return t('common.checkEveryMin', { n: minutes })
  const hours = minutes / 60
  return t('common.checkEveryHour', { n: hours })
}
...
export function HomePage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setTrips([])
      setError(e instanceof Error ? e.message : t('home.errorLoading', { error: '' }))
    }
  }
  ...
  const groups = useMemo(() => {
    ...
    for (const tr of items) {
      ...
      const destination = tr.destination ?? ''
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gray-900">{t('home.title')}</h2>
        <Link
          to="/search"
      ...
      {trips == null ? (
        <div className="rounded-lg bg-white p-4 shadow-sm text-sm text-gray-600">{t('home.loading')}</div>
      ) : error ? (
        <div className="rounded-lg bg-white p-4 shadow-sm text-sm text-red-700">
          {t('home.errorLoading', { error })}
        </div>
      ) : empty ? (
            ...
            <section key={g.key} className="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-renfe-purple/20">
              <div className="bg-renfeHeader px-4 py-3 text-white">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-white/85">{t('home.trackedCount', { count: g.trips.length })}</span>
                </div>
              </div>

              <ul className="divide-y divide-gray-100">
                {g.trips.map((tr) => {
                  const currentPrice = tr.current_price ?? null
                  const notPublished = currentPrice == null
                  const color = priceColor(currentPrice, tr.estimated_price_min ?? null, tr.estimated_price_max ?? null)
                  const direction = tr.last_price_change_direction ?? null
                  const dateLabel = [tr.date, tr.departure_time].filter(Boolean).join(' · ')

                  const rangeMin = tr.estimated_price_min
                  const rangeMax = tr.estimated_price_max
                  ...
                  const isPast = isTripPast(tr)

                  return (
                    <li key={tr.id} className={`px-4 py-4 ${isPast ? 'bg-red-50' : ''}`}>
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                            <div className="text-sm font-semibold text-gray-900">{tr.train_identifier}</div>
                            <div className="text-sm text-gray-600">{dateLabel}</div>
                          </div>
                          <div className="mt-1 text-xs text-gray-600">
                            <span className="font-medium text-gray-700">{t('home.check')}</span> {formatCheckInterval(t, tr.check_interval_minutes)}
                          </div>
                          {(hasMin || hasMax) && (
                            <div className="mt-1 text-xs text-gray-600">
                              {hasMin && hasMax ? (
                                <>
                                  <span className="font-medium text-gray-700">{t('common.estimatedPrice')}</span> {fmt(rangeMin!)} € – {fmt(rangeMax!)} €
                                </>
                              ) : hasMin ? (
                                <>
                                  <span className="font-medium text-gray-700">{t('common.estimatedPrice')}</span> {t('common.from')} {fmt(rangeMin!)} €
                                </>
                              ) : (
                                <>
                                  <span className="font-medium text-gray-700">{t('common.estimatedPrice')}</span> {t('common.upTo')} {fmt(rangeMax!)} €
                                </>
                              )}
                            </div>
                          )}
                          <div className="mt-1 text-xs text-gray-500">{t('home.lastCheckedAt', { date: formatLastCheckedAt(t, tr.last_checked_at) })}</div>
                        </div>

                        <div className="flex flex-row items-center justify-between gap-3 sm:flex-col sm:items-end">
                          <div className="text-right">
                            {notPublished ? (
                              <div className="text-sm italic text-gray-500">{t('home.notPublished')}</div>
                            ) : (
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            <Link to={`/trip/${tr.id}`} className="font-medium text-renfe-red hover:text-renfe-redHover">
                              {t('home.viewDetails')}
                            </Link>
                            <button
                              type="button"
                              onClick={() => setRemoveId(tr.id)}
      ...
      <Dialog open={removeId != null} onClose={() => (removing ? null : setRemoveId(null))} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl ring-1 ring-black/5">
            <DialogTitle className="text-base font-semibold text-gray-900">{t('home.removeTitle')}</DialogTitle>
            <p className="mt-2 text-sm text-gray-600">{t('home.removeBody')}</p>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setError(e instanceof Error ? e.message : t('home.errorCouldNotRemove'))
                    setRemoveId(null)
                  } finally {
                    setRemoving(false)
                  }
