...
# Configuration
# RENFE_GTFS_DIR: directory for GTFS data (default "renfe_schedule" in cwd).
# In Docker, set to e.g. /data/renfe_schedule so data persists.
RENFE_API_URL = "https://data.renfe.com/api/3/action/resource_show"
RESOURCE_ID = "25d6b043-9e47-4f99-bd91-edd51d782450"


def _gtfs_dir():
    return os.environ.get("RENFE_GTFS_DIR", "renfe_schedule")


def _zip_path():
    base = _gtfs_dir()
    if os.path.isabs(base):
        return os.path.join(os.path.dirname(base), "renfe_schedule.zip")
    return "renfe_schedule.zip"


def _metadata_file():
    return os.path.join(_gtfs_dir(), ".last_updated")


# Used as defaults; call _gtfs_dir() / _zip_path() / _metadata_file() for env-aware paths
LOCAL_DATA_DIR = "renfe_schedule"  # fallback only
...
def download_and_extract(download_url, last_modified):
    ...
    try:
        ...
        print(f"[OK] Downloaded {os.path.getsize(zip_path) / 1024 / 1024:.2f} MB")
        ...
        print(f"[EXTRACT] Securely extracting to {data_dir}/...")
        try:
            extracted_files = safe_extract_zip(zip_path, data_dir)
